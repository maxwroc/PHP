...
//			set_error_handler( array( $this, 'exception_handler' ) );
	...
	/**
	 * Returns instance of Database object for given config name
	 * 
	 * @access public
	 * @param string $sConfigName
	 * @return Database
	 */
	public function getDatabase( $sConfigName = 'default' ) {
		return Database::getInstance( $sConfigName );
	}
	 ...
	 */
	public function findFile( $sName, $sType = 'Controller', &$aPathsScanned = array() ) {
		
		// Remove sType from controller name
		$sName = str_replace( $sType . '_', '', $sName );
		
		// If type contains "_" replace them with directory separator char
		$sType = str_replace( '_', DIRECTORY_SEPARATOR, $sType );
		
		if ( $sType == 'i18n' ) $sType .= DIRECTORY_SEPARATOR . $this->sLanguage;
		
		return Loader::findFile( $sType . DIRECTORY_SEPARATOR . $sName . EXT, $aPathsScanned );
	...
	public function Dispatch() {
		...
		// Render view if such should be used
		try {
			$oController->getTemplate()->render( true );
		} catch ( Lithium_Exception $e ) {
			// We dont want to force user to use view layer
		}
		// run postDispatcher
    ...
    $aPathsScanned = array();
		if ( $this->findFile( $sControllerName, 'Controller', $aPathsScanned ) === false ) {
			throw new Lithium_404_Exception( 'Core.controller_not_found', $sControllerName, $this->oRouter->getApplicationName(), implode( "\n", $aPathsScanned ) );
		}
...
class LDateTime {
	 ...
	 * Class constructor
	 * 
	 * @param string/int $mDate
	 * @throws Lithium_404_Exception
	 */
	public function __construct( $mDate ) {
		$this->parseDate( $mDate );
	}
	
	/**
	 * Returns new instance of LDateTime
	 * 
	 * @param string/int $sDate
	 */
	public static function factory( $mDate ) {
		return new LDateTime( $mDate );
	}
	 ...
	 */
	public function format( $sFormat = 'Y-m-d' ) {
		return date( $sFormat, $this->aDate['unix'] );
	}
	...
	/**
	 * Checks if current date time is higher then given one
	 * 
	 * @param LDateTime $oDT
	 * @return bool
	 */
	public function isHigherThen( LDateTime $oDT ) {
	...
	/**
	 * Changes date time object using given string (must be compatible with strtotime)
	 * 
	 * Examples:
	 * 		+1 day
	 * 		+1 week
	 ...
	 * 		last Monday
	 * 
	 * @param string $sChangeString
	 */
	public function change( $sChangeString ) {
		$this->parseDate( strtotime( $sChangeString, $this->unix() ) );
	}
	 ...
	 */
	private function parseDate( $mDate ) {
		
		if ( is_int( $mDate ) && $mDate > 0 ) {
			$mDate = date( 'Y-m-d H:i:s', $mDate );
		}
		
		$this->aDate = date_parse( $mDate );
		if ( $this->aDate === false ) {
			throw new Lithium_Exception( 'updater.incorrect_date', $mDate );
		}
		
		$this->aDate['unix'] = strtotime( $mDate );
		$this->aDate['string'] = date( 'Y-m-d H:i:s', $this->aDate['unix'] );
	}
